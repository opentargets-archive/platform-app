...
const sections = [
  ...
  section =>
    (config.configProfile.hideDiseaseSectionIds.length === 0 ||
      !config.configProfile.hideDiseaseSectionIds.includes(
        section.definition.id
      )) &&
    (!section.definition.isPrivate ||
      (section.definition.isPrivate && config.configProfile.isPartnerPreview))
);
