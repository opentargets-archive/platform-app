...
- JSX Node rendered in the table cell: `renderCell`.

If `propertyField` is absent, all three fields must exist for the column.
...
| `exportValue`  | `false \| function` | Yes if `propertyPath` is absent | If present, the data downloader will use the return value for exporting this column. Otherwise, it will use `propertyPath`. If set to `false`, the column will not be present in the export.<br><br>**Signature**: `function(row: object) => string`<br>_row_: a row coming from the `row` table prop array.                                                                                                                                                                                                           |
| `filterValue`  | `function`          | Yes if `propertyPath` is absent | If present, the global filter will use the return value of this function to discriminate elements. Otherwise, it will use `propertyPath`.<br><br>**Signature**: `function(row: object) => string`<br>_row_: a row coming from the `row` table prop array.                                                                                                                                                                                                                                                              |
...
| `propertyPath` | `string`            | Yes \*                          | Path inside the `row` object where the data for this column will be retrieved from. If not present, this will fall back to the value of the `id` field.                                                                                                                                                                                                                                                                                                                                                                |
| `renderCell`   | `function`          | Yes if `propertyPath` is absent | If present, the table will render the return value of the function in the cells of the column.<br><br>**Signature**: `function(row: object) => node`<br>_row_: a row coming from the `row` table prop array.                                                                                                                                                                                                                                                                                                           |
...
**\*** Required if any of `exportValue`, `filterValue` and `renderCell` is not present.
